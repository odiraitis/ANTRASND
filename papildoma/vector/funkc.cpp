...
std::vector<mok> raskMinkstus(std::vector<mok>& studentai) {
    std::vector<mok> temp_lievi;
    std::vector<mok>::size_type i = 0;
    int kiek=0;
    // invariantas: vektoriaus `studentai` elementai [0, i) yra "kieti"
    for(i=0; i<studentai.size();i++)
    {
        if(studentai[i].gal<5 && studentai[i].galm<5)
        {
            temp_lievi.push_back(studentai[i]);
        }
        else
        {
    ...
    return temp_lievi;  // grąžina studentus gavusius skolą
}
